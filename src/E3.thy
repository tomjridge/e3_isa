...
definition pop_todo :: "
('set_todo_done,'map_blocked_key,'map_complete_key,'map_sym_sym_int_int,'map_tm_int) ty_loop2
=> (('set_todo_done,'map_blocked_key,'map_complete_key,'map_sym_sym_int_int,'map_tm_int) ty_loop2 * item) option" where
"pop_todo s0 = (
  case (todo5 s0) of
    [] => None (*(failwith ''pop_todo'') *)
  | x#xs => Some (s0 (| todo5 :=xs |),x))"
